...
pub fn show_tasks(tasks: &[Task]) {
    ...
    table.set_titles(row!["Id", "Priority", "Status", "Description"]);
    for t in tasks {
        table.add_row(row![&t.id.to_string(), &t.priority, &t.status, &t.descr]);
    }
    table.printstd();
}
...
pub fn show_tasks_labels(filename: &Path, tasks: &[Task]) {
    let mut table = Table::new();
    table.set_titles(row!["Id", "Priority", "Status", "Labels", "Description"]);
    for t in tasks {
